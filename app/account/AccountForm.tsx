...
import {
  User, Building2, Phone, Globe, Upload,
  CheckSquare, Square, Loader2, CheckCircle, Lock,
} from "lucide-react";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { error } = await supabase.from("users").update({
        ...
        roles: rolesArray,
        role: rolesArray.includes("seller") ? "seller" : "buyer",
    ...
    <div className="space-y-4">
      ...
      <div className="card p-4 flex flex-wrap gap-3">
        <div className={`flex items-center gap-2 text-xs font-medium px-3 py-1.5 rounded-full ${
          profile?.is_verified ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-600"
        }`}>
          {profile?.is_verified ? "✓ Verified" : "⏳ Pending Verification"}
        </div>
        <div className={`flex items-center gap-2 text-xs font-medium px-3 py-1.5 rounded-full ${
          profile?.is_subscribed ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-500"
        }`}>
      ...
      <form onSubmit={handleSubmit} className="card p-5 sm:p-7 space-y-6">
        ...
        <div>
          ...
          <input value={email} readOnly
            className="input min-h-[44px] bg-gray-50 text-gray-400 cursor-not-allowed text-sm" />
