...
import { fetchMessages, sendMessage } from "@/lib/services/chatService";
...
export default function ChatWindow({
  ...
  const poll = useCallback(async () => {
    try {
      const fresh = await fetchMessages(currentUserId, otherUserId, productId);
      setMessages(fresh);
    } catch { /* silent */ }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`chat-${productId}-${currentUserId}-${otherUserId}`)
      .on("postgres_changes", {
  ...
  }, [productId, currentUserId, otherUserId, poll]);
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    setText("");
    try {
      await sendMessage({ senderId: currentUserId, receiverId: otherUserId, productId, message: msg });
  ...
  return (
    /* Full-screen flex column — no fixed heights */
    <div className="flex flex-col min-h-screen md:min-h-0 md:h-[calc(100vh-4rem)] max-w-2xl mx-auto w-full">
      ...
      {/* Messages — flex-1 + overflow-y-auto = scrollable, fills remaining space */}
          ...
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[80%] sm:max-w-[70%] px-3 py-2 sm:px-4 rounded-2xl text-sm shadow-sm ${
                    isMine
      ...
      {/* Input — sticky at bottom, shrink-0 prevents compression */}
      <form
        onSubmit={handleSend}
