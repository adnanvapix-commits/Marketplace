...
export async function PATCH(req: NextRequest) {
  ...
  const { userId, is_subscribed, subscription_expiry, is_blocked, subscription_tier } = await req.json();
  ...
  if (is_blocked !== undefined)       action = is_blocked ? "block_user" : "unblock_user";
  else if (is_subscribed !== undefined) action = is_subscribed ? "activate_subscription" : "deactivate_subscription";
  else if (subscription_tier !== undefined) action = `set_tier_${subscription_tier ?? "none"}`;

  await db.from("admin_logs").insert({
    admin_id: admin.id,
    action,
    target_user_id: userId,
    details: { subscription_expiry, is_subscribed, is_blocked, subscription_tier },
  });
