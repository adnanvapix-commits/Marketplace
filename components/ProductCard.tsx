...
import { MapPin, Package, Tag, ArrowRight, Heart } from "lucide-react";
...
const ProductCard = memo(function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="card group flex flex-row items-stretch gap-0 overflow-hidden transition-all duration-200 hover:shadow-cream-md hover:-translate-y-0.5 hover:border-primary/20 relative">
      ...
      <button
        ...
        className={`absolute top-2 right-2 z-10 w-7 h-7 rounded-full flex items-center justify-center transition-all ${
          isWishlisted
            ...
            : "bg-white/80 text-gray-400 hover:bg-red-50 hover:text-red-400 border border-cream-200"
        }`}
      ...
      <Link href={`/product/${product.id}`} className="flex flex-row items-stretch gap-0 flex-1 min-w-0">
        {/* Image / placeholder */}
        ...
        {/* Content */}
        <div className="flex flex-col gap-1.5 flex-1 min-w-0 p-3 pr-9">
          {/* Title + condition */}
